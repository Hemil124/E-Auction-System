...
/* session_start();
  ...
  exit();
  } elseif (!isset($_SESSION['business_token'])) {
  ...
  $otp_code = isset($_SESSION['OTP']) ? $_SESSION['OTP'] : '';
  if (strlen($otp) === 5) {
  if ($otp_code == $otp) {
  ...
  function insertion() {
  $conn = mysqli_connect("localhost:3308", "root", "", "project1");
  if (!$conn) {
  ...
  $sql = "INSERT INTO tblregistration (FirstName, LastName, ContactNo, Email, DateofBirth, User_Type, Password) VALUES ('$fname', '$lname', '$contact', '$email', '$dob', '$user_type', '$hashed_pass')";
  ...
  function button1() {
  $otp_code = rand(1000, 99999);
  $email = $_SESSION["email"];
  $_SESSION["OTP"] = $otp_code;

  require 'C:\XAMPP\htdocs\banquet\PHPMailer-master\src\PHPMailer.php';
  require 'C:\XAMPP\htdocs\banquet\PHPMailer-master\src\Exception.php';
  require 'C:\XAMPP\htdocs\banquet\PHPMailer-master\src\SMTP.php';
  ...
  $mail->Password = 'mhumbqdtczqbctok';
  $mail->SMTPSecure = 'ssl';
  $mail->Port = 465;
